...
function readTeacherFavoriteMutationActor_() {
  ...
  if (!String(getTenantId_() || '').trim()) {
    throw new Error('tenant が未設定です。');
  }
  if (!configuredTeacherName && !configuredTeacherEmail) {
    throw new Error('教師設定が未完了です。');
  }
...
function resolveTeacherFavoriteTarget_(responseId, options) {
  ...
  const rosterExists = getRosterEntries_(true).some(student => {
  ...
  if (!existingRow || Number(existingRow.rowNumber || 0) < 2) {
    throw new Error('存在しない responseId は更新できません。');
  }
  return {
    response,
    lesson,
