...
- GAS と Pages は必要な側だけ公開する。
- `pages-release` が GitHub Pages の本番成果物 branch。
- `main` push 後の Pages 公開は GitHub Actions を正規経路とする。
- 同一 `source commit` に対して手動 publish を重ねない。
- teacher-only 変更では student 本体を変更しない。ただし `portable/student.html` への `source commit` 埋め込みだけの差分は許容する。
- 次だけでは Pages 本番反映完了と扱わない。
  `main` への push のみ、`codex/main-publish` への push のみ、portable build のみ、GAS deployment のみ。
- Pages 公開完了は `main` / `pages-release` / 公開 HTML の `source commit` 一致で判定する。
...
- `pages-release` 競合時は停止し、手動 rebase や手動競合解決はしない。
